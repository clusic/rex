...
module.exports = class KoaWorkerService extends Koa {
    ...
    await new Promise((resolve, reject) => {
      ...
      this.connection.listen(port, err => {
        ...
        utils.checkPortCanUse(port).then(_port => {
          ...
          if (this.cluster) {
            this.send('master', 'start:info', {
              data: {
                [chalk.bold(chalk.green('#'))]: chalk.magenta('✔'),
                [chalk.bold(chalk.green('Time'))]: chalk.gray(new Date().toLocaleString()),
                [chalk.bold(chalk.green('Process Port'))]: chalk.yellow(this.name),
                [chalk.bold(chalk.green('Server Port'))]: chalk.blue(port),
                [chalk.bold(chalk.green('Location'))]: chalk.gray('127.0.0.1'),
                [chalk.bold(chalk.green('View'))]: url
              }
            });
          } else {
  ...
  async [BuildComponents]() {
    ...
    for (let i = 0; i < controllers.length; i++) {
      ...
      params.forEach(param => $router.param(param.Name, RexMethod.RenderMiddlewareArguments(this.Middleware, param)));
      ...
      if (!prefix) continue;
      const ControllerPrepareMiddlewares = uses.slice(0).reverse().map(middle => RexMethod.RenderMiddlewareArguments(this.Middleware, middle));
